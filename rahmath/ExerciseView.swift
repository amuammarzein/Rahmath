...
struct ExerciseView: View {
    ...
    func answerCheck(option:Int){
        ...
        if(result == option){
            ...
            playSound(file_name: "incorrect.mp3")
            incorrect_answer += 1
            img = "rahmath_3"
            answer_status = false
            ...
            ScrollView(showsIndicators:false){
                ...
                VStack(spacing:20){
                    ...
                    ZStack(alignment: .leading){
                        Rectangle().frame(height:24).cornerRadius(20).padding(.bottom,10).foregroundColor(Color("colorGrey")).opacity(0.3)
                    ...
                    HStack(){
                        ZStack(){
                            ...
                            ZStack(){
                                Circle().foregroundColor(Color("colorWhite").opacity(0.5)).frame(height:40)
                                Text(String(num_1)).font(.system(size:20,weight: .heavy)).foregroundColor(Color("colorWhite")).frame(height:40)
                            }.offset(y:50)
                            
                        }
                        Image(systemName: op_icon).font(.system(size: 50, weight: .bold)).foregroundColor(Color("colorBlue")).frame(width:50)
                        ZStack(){
                            Rectangle().frame(height:150).cornerRadius(46 ).foregroundColor(Color("colorBlue"))
                ...
                VStack(){
                    VStack(){
                        ...
                        if(check_answer == true && num_question < limit_question){
                            ...
                            NavigationLink(destination: ResultView(correct_answer: correct_answer, num_question:num_question).navigationBarBackButtonHidden(true), isActive: $isActive) {
                    ...
                    }.frame(width:screenWidth,height:200).background(Color("colorWhite")).cornerRadius(40).padding(.top,-30)
                    VStack(spacing:30){
                            ...
                            HStack(spacing:30){
                                ...
                                ForEach((0..<2)) { index in
                                    Button(
                                        ...
                                        ZStack(){
                                            Rectangle().frame(height:95).cornerRadius(30).foregroundColor(Color("colorWhite")).shadow(color:Color("colorBlueDark"),radius: 0,x:1,y:5)
                                            Text(String(arr_answer_option[index].option)).font(.system(size: 50, weight: .heavy,design: .rounded)).foregroundColor(Color("colorBlack"))
                                        }
                            ...
                            HStack(spacing:30){
                                ForEach((0..<2)) { index in
                                    if(arr_answer_option[index].option==option_selected){
                                        ZStack(){
                                            Rectangle().frame(height:90).cornerRadius(30).foregroundColor(answer_status==true ? Color("colorGreen"):Color("colorWhite")).shadow(color:Color("colorBlueDark"),radius: 0,x:1,y:5) .overlay(RoundedRectangle(cornerRadius: 30).strokeBorder(Color("colorBlue"), lineWidth: 5))
                                                .padding(2)
                                                .overlay(RoundedRectangle(cornerRadius: 30).strokeBorder(Color("colorWhite"), lineWidth: 3))
                                            Text(String(arr_answer_option[index].option)).font(.system(size: 50, weight: .heavy,design: .rounded)).foregroundColor(Color("colorBlack"))
                                        }.opacity(1)
                                    }else{
                                        ZStack(){
                                            Rectangle().frame(height:95).cornerRadius(30).foregroundColor(arr_answer_option[index].option==result ? Color("colorGreen"):Color("colorWhite")).shadow(color:Color("colorBlueDark"),radius: 0,x:1,y:5)
                                            Text(String(arr_answer_option[index].option)).font(.system(size: 50, weight: .heavy,design: .rounded)).foregroundColor(Color("colorBlack"))
