        ...
        NavigationStack {
            ZStack {
                VStack {
                    ZStack {
                        ZStack {
                            Circle()
                                ...
                                .opacity(0.2)
                                .offset(x: (screenWidth * -1) / 2, y: (screenHeight * -1) / 6)
                            Circle()
                                .frame(height: 350)
                                .foregroundColor(Color("colorWhite"))
                                .opacity(0.2).offset(x: (screenWidth * 1) / 3.5, y: (screenHeight * 1) / 6)
                        }
                    ...
                    VStack(spacing: 20) {
                        Text("Revolution Math")
                            .font(.system(size: 32, weight: .bold, design: .rounded))
                            .foregroundColor(Color("colorBlack"))
                        Text("Let's understand math. Learn with us!")
                        ...
                        NavigationLink(destination: SelectHeroView().navigationBarBackButtonHidden(true)) {
                            Text("Next")
                                ...
                                .padding(8)
                                .frame(maxWidth: .infinity).shadow(radius: 0, x: 0, y: 0)
                ...
                withAnimation(.spring()) {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        withAnimation(.spring()) {
                            ...
                            withAnimation(
                                .interpolatingSpring(
                                    mass: 1.0, stiffness: 100.0, damping: 10.0, initialVelocity: 0)) {
                                done = true
                            }
                        }
                    }
